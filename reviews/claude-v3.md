...
| G6 | (v3.2, `reviews/gpt-v3.2.md`) Bekçi, güncelleme sırasında Axion'u yeniden açabilir. | Temel yol zaten kapalı: `guncelle.bat` Stop-Process ile durdurur, çıkış kodu -1, bekçi çıkar. Ek güvence (v3.3.0): yeniden başlatmadan önce ve 5 sn beklemeden sonra komut satırında `guncelle.bat` geçen `cmd.exe` varsa bekçi çıkar. GPT'nin önerdiği açık sinyal (bayrak dosyası) Axion durdurulmadan önce yazılmalıydı; bu da `guncelle.bat`'ın `git pull` öncesi satırlarını değiştirmek demek (cmd dosyayı çalışırken okur, güncelleme kendini bozar), o yüzden yapılmadı. Windows'ta çift tıklamayla doğrulanacak. |
| G7 | (v3.2) Akış durumu süreç genelinde: bir tabletin akışı diğerinin yedek görüntüsünü keser. | Doğru. Jeton oturuma bağlı (`stream.ticket(client)`, HMAC imzalı); "akış açık mı" oturum başına. Test (sahte jeton, iki tablet). |
| G8 | (v3.2) Alıcı görevin istisnası tüketilmiyor, kopma normal kapanış sayılmıyor. | Doğru. `WebSocketDisconnect` normal kapanış; `finally`'de görev beklenir, istisnası tüketilir, oturumun akış kaydı silinir. Test. |
