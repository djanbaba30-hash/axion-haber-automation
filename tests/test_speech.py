...
def test_speech_is_found_even_in_street_noise(tmp_path):
    ...
    subprocess.run(["ffmpeg", "-v", "error", "-y", "-i", str(SPEECH), "-f", "lavfi", "-i", "anoisesrc=color=pink:d=6:a=0.08:seed=1",
...
@pytest.mark.parametrize("name, source, extra", [
    ("trafik", "anoisesrc=color=brown:d=6:a=0.3:seed=1", ()),
    ("kalabalik", "anoisesrc=color=pink:d=6:a=0.3:seed=1", ("-af", "volume='0.6+0.4*sin(2*PI*0.7*t)':eval=frame")),
    ("carpma", "anoisesrc=color=white:d=6:a=0.5:seed=1", ("-af", "volume='if(lt(mod(t,1.3),0.15),1,0.1)':eval=frame")),
