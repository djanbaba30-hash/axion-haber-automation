...
10 dakikada bir yapılır; ulaşılamazsa panel yine açılır.
"""
...
def lines(api_key: str | None) -> list[str]:
    ...
    if "hata" in voice:
        rows.append(f"**ElevenLabs:** okunamadı ({voice['hata']})")
    else:
        warn = "⚠️ " if voice["kalan"] < 0.1 * max(1, voice["sinir"]) else ""
        renewal = f" · yenilenme {voice['yenilenme']}" if voice["yenilenme"] else ""
        rows.append(f"{warn}**ElevenLabs:** {voice['kalan']:,} karakter kaldı / {voice['sinir']:,}{renewal}".replace(",", "."))
    tools = ffmpeg()
    if tools["surum"]:
