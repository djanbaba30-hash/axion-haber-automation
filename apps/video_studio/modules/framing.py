...
def _profiles(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    return np.percentile(energy, 70, axis=0), np.percentile(energy, 70, axis=1)
...
def _bounds(profile: np.ndarray) -> tuple[float, float]:
    ...
    if np.median(profile[:first]) >= threshold or np.median(profile[last + 1:]) >= threshold:
        return 0.0, 1.0
    return start + SAFETY_INSET, end - SAFETY_INSET
